...
import { Predicate, PromiseEmitter } from "./types";
 ...
 * {@link RetryEvents}.
 */
export type Retry<T, E> = PromiseEmitter<RetryEvents<E>, T>;
...
class RetryImpl<T, E> implements Retry<T, E> {
  ...
  emit<Event extends keyof RetryEvents<E>>(
    event: Event,
    ...params: Parameters<RetryEvents<E>[Event]>
  ): boolean {
    return this.emitter.emit(event, ...params);
  }

  on<Event extends keyof RetryEvents<E>>(
    event: Event,
    handler: RetryEvents<E>[Event]
  ) {
