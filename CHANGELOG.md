# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [0.5.1] - 2025-12-30

### Fixed

- The typing of the base class the retry functionality inherits from. The previous typing
caused the type checking on the `emit` method to be unexistant. This is now fixed and the
bug, nor its fix, should have any impact in client code.

## [0.5.0] - 2025-12-26

### Changed

- The error type associated to the retry configuration now defaults to `unknown`. Although
`Error` was more convenient and is idiomatic JS code, `unknown` is more correct as JS allows
to throw anything, not just error types, and we should default to allow for the behavior
the platform allows.

## [0.4.1] - 2025-11-30

### Changed

- Updating code to use `@infra-blocks/emitter@0.2.1`. This shouldn't affect the public API.

## [0.4.0] - 2025-11-30

### Changed

- Setting main development node version to 22 LTS. Dropping support for node 18 in favor
of node 20.

## [0.3.0] - 2025-11-09

### Added

- The `retry` event: this one is only emitted starting on the second attempt, meaning
an error always occurred before this was invoked. This is a complement to the `attempt`
event, which is invoked on any attempt, including the first one (before any error could occur).

## [0.2.0] - 2025-11-08

### Added

- The `once` method on `Retry` objects. This was implemented for free by leveraging
[@infra-blocks/emitter](https://www.npmjs.com/package/@infra-blocks/emitter) instead of its
deprecated homologous in [@infra-blocks/types](https://www.npmjs.com/package/@infra-blocks/types)
package.

### Changed

- The [@infra-blocks/types](https://www.npmjs.com/package/@infra-blocks/types) dependencies moved
to development dependencies as it is now only used for type definitions.

## [0.1.2] - 2025-08-21

### Fixed

- The `retry` API now treats explicit `undefined` values the same as implicit `undefined` values.
Before this fix, if the options provided didn't contain a specific field, it would result in using
its corresponding default. When the field was provided explicitly with an `undefined` value, the
API would not use the corresponding default. The latter has been fixed so that the behavior is uniform
across `undefined` values.

## [0.1.1] - 2025-07-05

### Fixed

- Fixed legacy import of the module by adding a "main" field to the `package.json`.## [0.1.0] - 2025-06-17

### Added

- First implementation of the emitter like, promise like retry utility!

## [0.1.0] - 2025-06-17

### Added

- Initial release of the package containing the main `retry` utility function.

[0.5.0]: https://github.com/infra-blocks/ts-retry/compare/v0.4.1...v0.5.0
[0.4.1]: https://github.com/infra-blocks/ts-retry/compare/v0.4.0...v0.4.1
[0.4.0]: https://github.com/infra-blocks/ts-retry/compare/v0.3.0...v0.4.0
[0.3.0]: https://github.com/infra-blocks/ts-retry/compare/v0.2.0...v0.3.0
[0.2.0]: https://github.com/infra-blocks/ts-retry/compare/v0.1.2...v0.2.0
[0.1.2]: https://github.com/infra-blocks/ts-retry/compare/v0.1.1...v0.1.2
[0.1.1]: https://github.com/infra-blocks/ts-retry/compare/v0.1.0...v0.1.1
[0.1.0]: https://github.com/infra-blocks/ts-retry/releases/tag/v0.1.0
